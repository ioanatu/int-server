...
**Now:** none. A single client can trivially saturate a dyno.
...
On Heroku, ship logs and metrics off the dyno — the local filesystem is ephemeral.
...
database reachable) via `@nestjs/terminus`. Heroku's
health check should target readiness so a dyno is not sent traffic before it can serve.
...
**Change:** confirm in-flight requests drain within Heroku's 30-second `SIGTERM` window,
and add a short `server.keepAliveTimeout` above the router's (Heroku's is 90s) to avoid
502s from races on idle connections.

### 6.2 Secret management

**Now:** `SESSION_TOKEN` is a Heroku config var — correct for a PoC.
...
plus `docker-compose` makes local, CI and production identical, and removes the
Heroku-monorepo buildpack workaround the subdirectory layout currently needs.

### 6.4 Pin the toolchain

**Change:** `engines` pins Node `22.x`; add a `.nvmrc` and `packageManager` field so local
developers, CI and the Heroku build all resolve the same versions.
